...
|---------|------|--------------|
| 🏗️ Full | *(default)* | System deps → Python venv + `zyvor-qa` CLI → npm + Playwright Chromium → verify |
...
| Option | Effect |
|--------|--------|
| `--service` | Install + (re)start a systemd unit for `zyvor-qa serve` (full/quick/container profiles) |
...
Dashboard auth is on by default. Credentials default to **`admin` / `Admin@321`**, persisted per host in `~/.deployments/zyvor-qa-agent/.zyvor-qa-auth`, written into the remote `.env` and (on k3s) the Secret, and printed in the deploy summary. Override with `ZYVOR_QA_DASH_USER` / `ZYVOR_QA_DASH_PASS`, or `--no-auth` to disable.
...
- **State files** (`.zyvor-qa-port`, `.zyvor-qa-auth`, `.zyvor-qa-image-tag`) live under the remote deploy dir and are protected from `rsync --delete`, so the port, login, and image stay stable across redeploys.
...
- **k3s image freshness:** each `--k3s` deploy imports the image under a unique tag (`zyvor-qa-agent:d<timestamp>`) so containerd can't dedupe to stale code.
